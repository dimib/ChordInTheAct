...
final class AudioStreamManager: NSObject, ObservableObject {
    ...
    enum AudioStreamManagerState: Equatable {
        case idle
        case streaming(sampleTime: Int64)
        case error(message: String)
        static func ==(lhr: AudioStreamManagerState, rhr: AudioStreamManagerState) -> Bool {
            switch (lhr, rhr) {
    ...
    func start() throws {
        guard audioStreamManagerState == .idle else { return }
    ...
    func stop() {
        ...
        _audioStream = nil
        self.audioStreamManagerState = .idle
    }
}
